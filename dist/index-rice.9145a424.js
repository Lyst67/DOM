...
refs.raceBtn.addEventListener("click", ()=>{
    ...
    Promise.race(promises).then(({ horse, time })=>{
        updateWinnerField(`Перемогла коняка: "${horse}" з результатом: ${time}секунд!`);
        console.log(`%c Перемогла коняка: "${horse}" з результатом: ${time}секунд!`, "color: lightgreen; font-size: 14px;");
        updateWinnerTable({
            horse,
            time,
            raceCount
    ...
    Promise.all(promises).then((result)=>{
        ...
        console.log("%c Забіг закінчено!", "color: lightblue; font-size: 14px;", result);
    });
});
...
//# sourceMappingURL=index-rice.9145a424.js.map
